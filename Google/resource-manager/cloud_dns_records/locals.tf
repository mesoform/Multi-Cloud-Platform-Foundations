locals {
  ...
  cloud_dns_specs = {
    for zone, record_type in local.cloud_dns_components_specs :
      zone => merge(local.cloud_dns_components_common, record_type)
  }

  cloud_dns_records = {
    for zone, record_type in local.cloud_dns_components_specs: zone => {
